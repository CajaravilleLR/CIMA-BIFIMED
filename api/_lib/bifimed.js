...
function parseBifimedIndicaciones(html) {
  ...
  if (!dataRows.length) dataRows = $(target).find("tr").slice(1);
...
async function getByCnWithIndicaciones(cn) {
  ...
  if (!exact) return { sourceUrl: url, resultado: null };
  ...
  return {
    sourceUrl: url,
    resultado: { ...exact, detalleUrl, indicaciones }
  };
}
